                ...
                <div class="button-container">
                    {if ($product.id_product_attribute == 0 || (isset($add_prod_display) && ($add_prod_display == 1))) && $product.available_for_order && !isset($restricted_country_mode) && $product.customizable != 2 && !$PS_CATALOG_MODE}
                        {if (!isset($product.customization_required) || !$product.customization_required) && ($product.allow_oosp || $product.quantity > 0)}
                            ...
                            <a class="ajax_add_to_cart_button btn btn-outline-dark btn-sm"
                        ...
                        {else}
                            <span class="ajax_add_to_cart_button btn btn-outline-dark btn-sm disabled"><span>{l s='Add to cart'}</span></span>
                        {/if}
                    {/if}
                </div>
                        ...
                        <div class="compare">
                            <a class="add_to_compare" title="{l s='Add to Compare'}" href="{$product.link|escape:'html':'UTF-8'}"
                               data-id-product="{$product.id_product}">
                                <i class="fa fa-plus"></i> {l s='Add to Compare'}
                            </a>
                        </div>
                    {/if}
                </div>
                ...
                {if isset($product.available_for_order) && $product.available_for_order && !isset($restricted_country_mode)}
                    <div class="availability">
                        {if ($product.allow_oosp || $product.quantity > 0)}
							<span class="badge {if $product.quantity <= 0 && isset($product.allow_oosp) && !$product.allow_oosp} bg-danger{elseif $product.quantity <= 0} bg-warning{else} bg-success{/if}">
                        ...
                        {elseif (isset($product.quantity_all_versions) && $product.quantity_all_versions > 0)}
                            <span class="badge bg-warning">{l s='Product available with different options'}</span>
                        {else}
                            <span class="badge bg-danger">{l s='Out of stock'}</span>
                        {/if}
                    </div>
                {/if}
            {/if}
