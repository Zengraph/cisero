...
{else}
  <h3 class="mb-4">{l s='Bank-wire payment' mod='bankwire'}</h3>
  <form action="{$link->getModuleLink('bankwire', 'validation', [], true)|escape:'html'}" method="post">
    ...
    <p>
      ...
      {if $currencies|@count > 1}
        ...
        <select class="d-inline form-select" style="width:auto;" id="currency_payement" name="currency_payement" onchange="setCurrency($('#currency_payement').val());">
    ...
    <p class="cart_navigation mt-4" id="cart_navigation">
      <input type="submit" value="{l s='I confirm my order' mod='bankwire'}" class="btn btn-lg btn-dark me-5 mb-3 mb-md-0"/>
      <a href="{$link->getPageLink('order', true, NULL, "step=3")|escape:'html'}" class="btn btn-lg btn-outline-dark">{l s='Other payment methods' mod='bankwire'}</a>
    </p>
  </form>
{/if}
