<tr id="product_{$product.id_product}_{$product.id_product_attribute}_{if $quantityDisplayed > 0}nocustom{else}0{/if}_{$product.id_address_delivery|intval}{if !empty($product.gift)}_gift{/if}" class="cart_item address_{$product.id_address_delivery|intval} {if $odd}odd{else}even{/if} text-center align-middle">
  ...
  <td class="cart_description text-start">
    ...
    {if isset($product.attributes) && $product.attributes}<small class="cart_att">{if $product.reference}<br>{/if}<a href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute, false, false, true)|escape:'html':'UTF-8'}">{$product.attributes|@replace: $smarty.capture.sep:$smarty.capture.default|escape:'html':'UTF-8'}</a></small>{/if}
  </td>
  {if $PS_STOCK_MANAGEMENT}
    <td class="cart_avail">
		<div class="badge {if $product.quantity_available <= 0 && isset($product.allow_oosp) && !$product.allow_oosp} bg-danger{elseif $product.quantity_available <= 0} bg-warning{else} bg-success{/if} position-relative">{if $product.quantity_available <= 0}{if isset($product.allow_oosp) && $product.allow_oosp}{if isset($product.available_later) && $product.available_later}{$product.available_later}{else}{l s='In Stock'}{/if}{else}{l s='Out of stock'}{/if}{else}{if isset($product.available_now) && $product.available_now}{$product.available_now}{else}{l s='In Stock'}{/if}{/if}
		{if $product.quantity_available > 0 && !empty($ctheme.product_line.quantity.display)} 
			<span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
				{$product.quantity_available|intval}+
				<span class="visually-hidden">articles currently in stock</span>
			</span>
		{/if}
		</div>
