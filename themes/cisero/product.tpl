  ...
  <div itemscope itemtype="https://schema.org/Product">
    ...
    <div class="primary_block row">
      ...
      <div class="pb-left-column col-xs-12 col-sm-5 col-lg-6">
	    <h1 itemprop="name" class="prodtitle">{$product->name|escape:'html':'UTF-8'}</h1>
        ...
        {if !empty($images)}
          <div id="views_block" class="text-center {if isset($images) && count($images) < 2}hidden{/if}">
            <div id="thumbs_list">
                    ...
                    >
                      {if $jqZoomEnabled && $have_image && !$content_only}
                        ...
                        >
                          ...
                          <picture class="img-fluid" id="thumb_{$image.id_image|intval}">
                            {if !empty($webp)}
      ...
      <div class="pb-center-column col-xs-12 col-sm-7 col-lg-6">
          ...
          <form id="buy_block"{if $PS_CATALOG_MODE && !isset($groups) && $product->quantity > 0} class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html':'UTF-8'}" method="post">
            ...
            <div class="box-info-product">
                    ...
                    {foreach from=$groups key=id_attribute_group item=group}
                      {if !empty($group.attributes)}
                        <fieldset class="attribute_fieldset form-group">
                          <label class="form-label" {if $group.group_type != 'color' && $group.group_type != 'radio'}for="group_{$id_attribute_group|intval}"{/if}>{$group.name|escape:'html':'UTF-8'}&nbsp;</label>
                          {assign var="groupName" value="group_$id_attribute_group"}
                          <div class="attribute_list">
                            {if ($group.group_type == 'select')}
                              <select name="{$groupName}" id="group_{$id_attribute_group|intval}" class="form-select mb-3 attribute_select no-print">
                                {foreach from=$group.attributes key=id_attribute item=group_attribute}
                                  <option value="{$id_attribute|intval}"{if (isset($smarty.get.$groupName) && $smarty.get.$groupName|intval == $id_attribute) || $group.default == $id_attribute} selected="selected"{/if} title="{$group_attribute|escape:'html':'UTF-8'}">{$group_attribute|escape:'html':'UTF-8'}</option>
                                {/foreach}
                              </select>
                            {elseif ($group.group_type == 'color')}
                              <ul id="color_to_pick_list" class="justify-content-left">
              ...
              <div class="box-cart-bottom">
                {if !$PS_CATALOG_MODE}
                  <div id="quantity_wanted_p"{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || $PS_CATALOG_MODE} style="display: none;"{/if} class="mb-4">
                    <div><label for="quantity_wanted" class="form-label">{l s='Quantity'}</label></div>
                    <div class="input-group">
						...
						<input type="tel" min="1" name="qty" id="quantity_wanted" class="text-center form-control input-number" value="{if isset($quantityBackup)}{$quantityBackup|intval}{else}{if $product->minimal_quantity > 1}{$product->minimal_quantity}{else}1{/if}{/if}">
						<button class="btn btn-outline-secondary" type="button">
		...
		{if !empty($product->reference) && $product->reference}
			<p id="product_reference" class="mt-5">
			  <b>{l s='Reference'} :</b>
    ...
    {if !$content_only}
	  ...
	  {if isset($product) || !empty($features)}
		<hr class="border-4 border-top border-secondary mt-5 mb-5">
	  {/if}
	  
      {if isset($product) && $product->description}
        <section id="product-description" class="page-product-box mb-5">
                  ...
                  <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                      ...
                      <a class="btn btn-default btn-block" href="{$link->getPageLink('attachment', true, NULL, "id_attachment={$attachment.id_attachment}")|escape:'html':'UTF-8'}">
                        <i class="fa fa-download"></i> {l s="Download"} ({Tools::formatBytes($attachment.file_size, 2)})
                      </a>
                    </div>
                  </div>
                </div>
            ...
            <form method="post" action="{$customizationFormTarget}" enctype="multipart/form-data" id="customizationForm">
              ...
              {if $product->uploadable_files|intval}
                <div class="customizableProductsFile">
                  <h3>{l s='Pictures'}</h3>
                  <ul id="uploadable_files" class="list-unstyled">
